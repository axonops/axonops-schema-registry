...
// extractAvroFields extracts fields from an Avro schema.
...
func walkAvroSchema(s avrolib.Schema, prefix string, fields *[]FieldInfo) {
	...
	case *avrolib.RecordSchema:
		for _, f := range v.Fields() {
			...
			// In Avro, fields are required unless they are a union with null
...
// extractJSONSchemaFields extracts fields from a JSON Schema.
...
// extractProtobufFields extracts fields from a Protobuf schema using text parsing.
func extractProtobufFields(schemaStr string) []FieldInfo {
	var fields []FieldInfo
	// Simple regex-based extraction for protobuf fields
	// Matches patterns like: string name = 1;  int32 age = 2;  optional bool active = 3;
